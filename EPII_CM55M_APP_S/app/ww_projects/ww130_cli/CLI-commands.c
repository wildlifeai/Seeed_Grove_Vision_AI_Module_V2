...
static BaseType_t prvSnapshot(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString);
...
/* Structure that defines the "snapshot" image command line command. */
static const CLI_Command_Definition_t xSnapshot = {
	"snapshot", /* The command string to type. */
	"snapshot:\r\n Captures an image and exports to the SD card\r\n",
	prvSnapshot, /* The function to run. */
	0			 /* No parameters expected */
};
...
static BaseType_t prvSnapshot(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	...
	// Send message to camera task
	send_msg.msg_event = APP_MSG_IMAGEEVENT_STARTCAPTURE;
	send_msg.msg_data = 0;

	if (xQueueSend(xImageTaskQueue, (void *)&send_msg, __QueueSendTicksToWait) != pdTRUE)
	{
		xprintf("Failed to send 0x%x to Snapshot Task\r\n", send_msg.msg_event);
	}
	pcWriteBuffer += snprintf(pcWriteBuffer, xWriteBufferLen, "Capturing image");
}
...
static void vRegisterCLICommands(void)
{
	...
	FreeRTOS_CLIRegisterCommand(&xSnapshot);
}
