...
# firmware image (gen_image, device_image). Split out of ww500_minimal.mk to keep
# that file to just the build-path fix (D:\hxbuild) and the .elf staging
# step (stage_elf).
#
# Lives in mk/, not directly in ww500_minimal/, so ww.mk's own project-loader
# (which auto-includes every *.mk file directly in ww500_minimal/, non-recursive)
# does not also pick this up - it must only be included the one, deliberate
# way below, from the end of ww500_minimal.mk. Also depends on the 'stage_elf' target
# and on EPII_ROOT/HOST_OS/PS/CP/ECHO/Q (from options/scripts.mk), both
# already available by the time ww500_minimal.mk includes this file.
##
...
#   V  = image type: always M (ww500_minimal has no camera variants)
