...
## \* Firmware Artifact Set

    Purpose: Defines the software running on the two microprocessors (BLE MCU + Himax AI MCU).

#### Included Artifacts

- `ble_firmware.zip` - Runs on MKL62BA (BLE and LoRaWAN processor)

  - Example: `WildlifeWatcher_1_ww500_a00_000804.zip`
  - Version number indicated in filename (e.g., 000804)
  - Installed via DFU mechanism (see WW130_DFU.md)
  - DFU process prevents installation of earlier versions
- `ouput.img` - Runs on Himax AI processor

  - Generated during compilation process
  - Loaded using bootloader (see Compile_and_flash.md, section 4)

* **Machine Learning (ML) Artifacts**: Retrieved from the Model Zoo, the AI capabilities are defined by the ML model and its accompanying documentation.

  `Manifest.zip:`

  - **ML Model**: `/MANIFEST/model_vela.tfl` - The trained neural network model that identifies species in images.
  - **Model Manuscript & Labels**: `/MANIFEST/labels.txt` A document describing the model, its training data, and a `labels.txt` file that maps the model's output to species names.
* **Configuration File (`/MANIFEST/CONFIG.TXT`)**: A source file that defines default device parameters like `Delay before DPD` and `Num pics when triggered`.
...
The `CONFIG.TXT` file is stored on the SD card, included in the Manifest.zip folder. It's purpose is for register and sensor confirugation settings and device recovery data.

- **Interaction**: These compiled-in values serve as the device's default settings and get updated on boot. The mobile app can override them at runtime by sending specific BLE commands.
...
- **Storage**: There is no central automated registry. Artifacts are built locally by developers and manually placed in Supabase.
