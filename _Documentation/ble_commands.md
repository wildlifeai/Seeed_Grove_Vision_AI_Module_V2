...
This document describes these commands. Some commands are now redundant and can be removed. Others are useful 
for developers but should not be offered to users. 

There is scope for adding additional commands. It is possible to change the command syntax and responses - developers should request that.
...
struct CmdStruct appCommands[] = {
		...
		{"device", processDevice},					// Product name, such as 'WW500-C00'
		{"ver", processVer},						// Send device, firmware version, build date
		{"join", processJoin},						// Request a LoRaWAN join
...
The two tables below list all of the commands. 

The first table has the commands that are most likely to be of use to the app developer. 

The command set has evolved over time and includes commands  that are no longer useful, or 
are useful only for engineering purposes and should not be made available to non-engineering users.
These are listed in the second table.

The "Reqd?" column indicates whether the command should be implemented by the app, or notes below. 

__Most Useful Commands__
...
| getops         |               | Requests all the Operational Parameters       | Y     |
| selftest       |               | Requests the self test bit map as a hex number| Y     |
|                |               | Response: "Error bits = 0x0800"               |       |
| flashr         | n, m          | Flash red LED <n> times with<m> ms period     | Y     |
| flashg         | n, m          | Flash green LED <n> times with<m> ms period   | Y     |
| flashb         | n, m          | Flash blue LED <n> times with<m> ms period    | Y     |

__Redundant or Engineering-Only Commands__
...
| state          |               | Returns the state machine state               | 2     |
...
by typing at the AI processor console. 
The same commands can be sent by the app, preceded by the `'AI '` characters.
...
| AI getop       | n             | Requests the value of Operational Parameter n | Y, 1  |
...
|-----------|-------------------------------|----------|------------------------------------------------------|
| Wake      |                               |          | Sent by AI processor when it leaves DPD.             |
| Sleep     |                               |          | Sent by AI processor when it enters DPD.             |
...
* After power-on and until the BLE processor gets the time from the app or LoRaWAN its time starts at midnight on 1 Jan 2024.  
...
The app should disconnect from the device after say 1 minute without meaningful activity.
