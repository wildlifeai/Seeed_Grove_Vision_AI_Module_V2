...
|    22 | OP_PARAMETER_MD_FLASH_BRIGHTNESS_PERCENT | 50         | Brightness of the motion-detection illumination (percent; 16 hardware levels). Default raised from 5 (too dim for night MD in the field); the LED remains STROBE-gated to each MD frame's ~15 ms integration window, so this scales pulse amplitude, not duty cycle |
...
|    32 | OP_PARAMETER_CAM_RESOLUTION 			| 0             | RP camera capture resolution: 0 = 640x480 (normal pipeline), 1 = high resolution (encoded output 1216x960) single JPEG via the CPU pipeline - requires the NN off (op14 = 0); see [hires-capture.md](hires-capture.md) |
