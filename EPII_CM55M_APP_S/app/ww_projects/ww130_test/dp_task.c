 ...
 *
 * This is registered as a callback by cisdp_dp_init() in main_task.c
...
/*!
 * FreeRTOS task for processing messages from the data path
 *
 */
void dp_task(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(xDPTaskQueue, &(dp_recv_msg), __QueueRecvTicksToWait) == pdTRUE)
        {
            ...
            old_state = g_dptask_state;

            switch (dp_recv_msg.msg_event)
            {

            case APP_MSG_DPEVENT_CHANGE_STATE:
                // main_task changes our state
                if (dp_recv_msg.msg_data <= APP_DP_TASK_STATE_ERROR)
                {
                    g_dptask_state = dp_recv_msg.msg_data;
                }
                break;
            ...
            case APP_MSG_DPEVENT_XDMA_FRAME_READY:
                ...
                // CGP this was commented out
                // cisdp_get_jpginfo(NULL, NULL);
                g_cur_jpegenc_frame++;
                g_frame_ready = 1;
            ...
            if (old_state != g_dptask_state)
            {
                // state has changed
                XP_LT_CYAN;
                dbg_printf(DBG_LESS_INFO, "  dptask_state ");
                XP_WHITE;
